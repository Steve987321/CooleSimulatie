...
/// @brief prevents fluid from 'leaking' out
void sim::Physics::SetBnd(int b, float x[], int N)
{
	for (int i = 1; i < N - 1; i++)
	{
...
/// @brief solving some linear equantion for diffuse and project
void sim::Physics::LinSolve(int b, float x[], float x0[], float a, float c, int iter, int N)
{
	float cRecip = 1.0 / c;
...
/// @brief spreading of water
void sim::Physics::Diffuse(int b, float x[], float x0[], float diff, float dt, int iter, int N)
{
	float a = dt * diff * (N - 2) * (N - 2);
	this->LinSolve(b, x, x0, a, 1 + 6 * a, iter, N);
}

/// @brief fixes every square so the fluid stays constant
void sim::Physics::Project(float vx[], float vy[], float p[], float div[], int iter, int N)
{
	for (int j = 1; j < N - 1; j++)
	{
		for (int i = 1; i < N - 1; i++)
...
/// @brief handeling the velocities
void sim::Physics::Advect(int b, float d[], float d0[], float vx[], float vy[], float dt, int N)
{
	float i0, i1, j0, j1;
